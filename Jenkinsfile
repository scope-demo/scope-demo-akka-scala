        ...
        stage('Test'){
            parallel {
                stage('JDK11') {
                    steps {
                        sh 'echo test'
                        sh 'IS_CI=true JDK=11-jdk JAVA_PROFILE=java11 COMMIT=${GIT_COMMIT} docker-compose -p 11-jdk-${GIT_COMMIT} up --exit-code-from=scope-demo-akka-scala scope-demo-akka-scala'
                    }
                }
            }
        }
